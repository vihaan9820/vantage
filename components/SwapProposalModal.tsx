...
export function SwapProposalModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md animate-fade-in">
      <div className="relative w-full max-w-xl glass-panel rounded-3xl p-6 md:p-8 shadow-2xl border border-white/15 overflow-hidden flex flex-col gap-6">
        {/* Header & Steps */}
        <div className="flex justify-between items-start">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold text-[#10B981] bg-[#10B981]/10 border border-[#10B981]/20 px-2.5 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-1">
        ...
        <div className="w-full bg-white/10 h-1.5 rounded-full overflow-hidden flex">
          <div
            className="bg-gradient-to-r from-[#6366F1] to-[#10B981] h-full transition-all duration-300"
            ...
            <div className="flex flex-col gap-4 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {(partner.teachingSkills || []).map((item) => (
                  <button
                    ...
                    className={`p-3 rounded-2xl text-left border transition-all flex items-center justify-between ${
                      requestSkill === item.skill
                        ? "bg-[#6366F1]/20 border-[#6366F1] text-white shadow-lg shadow-[#6366F1]/10"
                  ...
                  >
                    ...
                    {requestSkill === item.skill && <Check size={16} className="text-[#10B981]" />}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-1.5 mt-2">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-white focus:outline-none focus:border-[#6366F1]"
                />
              </div>
            </div>
          )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {myTeachSkills.map((skill) => (
                  <button
                    ...
                    className={`p-3 rounded-2xl text-left border transition-all flex items-center justify-between ${
                      offerSkill === skill
                        ? "bg-[#10B981]/20 border-[#10B981] text-white shadow-lg shadow-[#10B981]/10"
                        : "bg-white/5 border-white/10 text-gray-300 hover:bg-white/10"
                    }`}
                  >
                    <span className="text-xs font-bold text-white">{skill}</span>
                    {offerSkill === skill && <Check size={16} className="text-[#10B981]" />}
                  </button>
                ))}
              </div>
                ...
                {[
                  ...
                  <button
                    ...
                    className={`p-4 rounded-2xl text-left border transition-all flex items-start gap-3.5 ${
                      format === item.val
                        ? "bg-[#6366F1]/20 border-[#6366F1] text-white shadow-lg"
                        : "bg-white/5 border-white/10 text-gray-300 hover:bg-white/10"
                    }`}
                  >
                    <item.icon size={20} className={format === item.val ? "text-[#10B981]" : "text-gray-400"} />
                    <div className="flex-1">
                      <p className="text-sm font-bold text-white">{item.title}</p>
                      <p className="text-xs text-gray-400 leading-relaxed mt-0.5">{item.desc}</p>
                    </div>
                    {format === item.val && <Check size={18} className="text-[#10B981]" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="flex flex-col gap-4 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                {(partner.slots || ["Today · 7:00 PM", "Tomorrow · 6:00 PM", "Saturday · 2:00 PM"]).map((slot) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Calendar size={14} className={selectedSlot === slot ? "text-[#10B981]" : "text-gray-400"} />
                      <span className="text-xs font-bold text-white">{slot}</span>
                    </div>
                    {selectedSlot === slot && <Check size={16} className="text-[#10B981]" />}
                  </button>
                ))}
              </div>
              ...
              <div className="flex flex-col gap-1.5 mt-2">
                ...
                <textarea
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl p-3 text-xs text-white focus:outline-none focus:border-[#6366F1]"
                />
              </div>
            </div>
          )}
            ...
            <div className="flex flex-col gap-4 animate-fade-in">
              ...
              <div className="bg-white/5 border border-white/10 rounded-2xl p-4 flex flex-col gap-3">
                <div className="flex justify-between items-center pb-3 border-b border-white/10">
                  ...
                  <span className="text-[11px] font-bold text-[#10B981] bg-[#10B981]/15 px-2.5 py-1 rounded-full border border-[#10B981]/25">
                    ⇄ Direct Barter
                  </span>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div className="bg-black/30 p-2.5 rounded-xl border border-white/5">
                    ...
                    <p className="font-bold text-[#10B981]">{offerSkill}</p>
                  </div>
                </div>

                <div className="flex justify-between items-center text-xs text-gray-300 pt-1">
                  <span className="flex items-center gap-1.5">
                    <Video size={14} className="text-[#6366F1]" /> {format}
                  </span>
                  <span className="flex items-center gap-1.5">
                    <Calendar size={14} className="text-[#10B981]" /> {selectedSlot}
                  </span>
                </div>
              </div>
            </div>
        ...
        <div className="flex justify-between items-center pt-4 border-t border-white/10">
          ...
          {step < 5 ? (
            ...
            >
              Next Step <ArrowRight size={14} className="text-black" style={{ color: "#000000" }} />
            </button>
          ) : (
            <button
            ...
            >
              <Zap size={14} className="text-black fill-black" style={{ color: "#000000", fill: "#000000" }} /> {isSubmitting ? "Sending..." : "Trade Skill · Send Proposal"}
            </button>
          )}
        </div>
      </div>
