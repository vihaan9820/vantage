...
const prepColors = (input?: string[]) => {
  const base = (input && input.length ? input : DEFAULT_NAVY_PALETTE).slice(0, MAX_COLORS);
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (uInvert > 0.5) {
    // Light mode: Pure white canvas background is primary, dynamic black fluid ripples are secondary
    float peakIntensity = clamp(ltn, 0.0, 1.0);
    vec3 darkSpike = mix(vec3(0.0, 0.0, 0.0), col, 0.35);
    float darkAlpha = clamp(peakIntensity * 1.3, 0.0, 1.0);
    fragColor = vec4(darkSpike, darkAlpha * uOpacity);
  } else {
    // Dark mode: True black background is primary, luminous white fluid spikes are secondary
    vec3 outc = col * ltn;
...
export interface FerrofluidProps {
  ...
  shimmer?: number;
  glow?: number;
  flowDirection?: 'up' | 'down' | 'left' | 'right' | string;
  opacity?: number;
...
export const Ferrofluid: React.FC<FerrofluidProps> = ({
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[Ferrofluid] WebGL initialization skipped or failed:', err);
      return;
    }
    ...
    try {
      ...
      console.warn('[Ferrofluid] WebGL program creation failed:', err);
      return;
    }
    ...
    const targetElement: (EventTarget & { addEventListener: Function; removeEventListener: Function }) | null =
    ...
    if (mouseInteraction && targetElement) {
      targetElement.addEventListener('pointermove', onPointerMove as EventListener, { passive: true });
    }
