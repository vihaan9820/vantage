...
import { ArrowRight, Calendar, Check, Clock, Sparkles, Video, X, Zap, Gem, AlertCircle } from "lucide-react";
...
export function SwapProposalModal({
  ...
  const handleBarterSubmit = () => {
    ...
    setTimeout(() => {
      ...
      toast.success(
        `🎉 Trade skill session scheduled with ${partner.name} for ${selectedSlot}! Added to your Sessions timeline.`
      );
      onClose();
      setStep(1);
    }, 400);
  ...
  const handleGemBookingSubmit = () => {
    if (!hasEnoughGems) {
      toast.error(`You need ${sessionPrice} Gems to book this session. Currently have ${walletGems} Gems.`);
      return;
    }
    setIsSubmitting(true);
    setTimeout(() => {
      const ok = book(partner.id, selectedSlot, sessionPrice, requestSkill);
      setIsSubmitting(false);
      if (ok) {
        toast.success(
          `🎉 Session confirmed with ${partner.name} for ${selectedSlot}! 💎 ${sessionPrice} Gems reserved.`
        );
        onClose();
      } else {
        toast.error("Failed to complete booking. Please check your Gems balance.");
      }
    }, 400);
  };
      ...
      <div className="relative w-full max-w-xl max-h-[92vh] overflow-y-auto glass-panel rounded-2xl sm:rounded-3xl p-5 sm:p-6 md:p-8 shadow-2xl border border-white/15 flex flex-col gap-5 sm:gap-6">
        ...
        {bookingType === "gems" ? (
          ...
          <div className="flex flex-col gap-4 animate-fade-in">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                {(partner.teachingSkills || []).map((item) => (
                  ...
                  >
                    ...
                    {requestSkill === item.skill && <Check size={14} className="text-white" />}
                  </button>
                ))}
              </div>
              <input
                ...
                placeholder="Or specify custom learning topic..."
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {(partner.slots || ["Today · 7:00 PM", "Tomorrow · 6:00 PM", "Saturday · 2:00 PM"]).map((slot) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      <Calendar size={13} className={selectedSlot === slot ? "text-white" : "text-gray-400"} />
                      <span className="text-xs">{slot}</span>
                    </div>
                    {selectedSlot === slot && <Check size={14} className="text-white" />}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div>
              <label className="text-xs font-bold text-white block mb-1.5">
                3. Session Interaction Format
              </label>
              <div className="grid grid-cols-3 gap-2 text-xs">
                {[
                  { label: "1 hr Live Video", icon: Video },
                  { label: "2x 30-min Reviews", icon: Clock },
                  { label: "Async Code Review", icon: Sparkles },
                ].map((item) => (
                  <button
                    key={item.label}
                    type="button"
                    onClick={() => setFormat(item.label as any)}
            ...
            {/* Gems Wallet Status Card */}
            <div className="p-4 rounded-2xl bg-white/[0.04] border border-white/15 flex flex-col gap-2">
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">Available Wallet Balance:</span>
                <strong className="text-white font-mono">{walletGems} Gems</strong>
              </div>
              <div className="flex justify-between items-center text-xs">
                <span className="text-zinc-400">Session Rate:</span>
                <strong className="text-white font-mono">💎 {sessionPrice} Gems</strong>
              </div>
              ...
              {hasEnoughGems ? (
                ...
                <div className="flex flex-col gap-2 pt-1">
                  <div className="flex items-center gap-2 text-xs text-amber-400">
                    ...
                    <span>Need {sessionPrice - walletGems} more Gems to book this session.</span>
                  </div>
                  <button
                    type="button"
                    onClick={() => {
                      claimStarterReward();
                      toast.success("★ +20 Starter Gems claimed to your wallet!");
                    }}
                    className="p-2 rounded-xl bg-white/15 hover:bg-white/25 border border-white/30 text-white text-xs font-bold text-center transition-all flex items-center justify-center gap-1.5"
                  >
                    <Sparkles size={13} />
                    <span>Claim +20 Starter Gems</span>
                  </button>
                </div>
              )}
            </div>
            ...
            <div className="flex justify-between items-center pt-2 border-t border-white/10">
              ...
              <button
                type="button"
                onClick={handleGemBookingSubmit}
                disabled={isSubmitting || !hasEnoughGems}
                style={{ backgroundColor: "#FFFFFF", color: "#000000", border: "1px solid #FFFFFF" }}
                className="primary-action bg-white text-black hover:bg-zinc-200 border border-white text-xs py-2.5 px-6 flex items-center gap-2 font-bold shadow-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
              >
                <Gem size={14} className="text-black fill-black" style={{ color: "#000000", fill: "#000000" }} />
                <span style={{ color: "#000000", fontWeight: 800 }}>
                  {isSubmitting ? "Confirming..." : `Confirm Booking · 💎 ${sessionPrice} Gems`}
                </span>
              </button>
            </div>
          </div>
          ...
          /* =========================================================================
             MUTUAL 1:1 BARTER SWAP FLOW (0 GEMS)
             ========================================================================= */
          <>
            {/* Step Progress Bar */}
                    ...
                    {(partner.teachingSkills || []).map((item) => (
                      <button
                        ...
                        className={`p-3 rounded-2xl text-left border transition-all flex items-center justify-between ${
                          requestSkill === item.skill
                            ? "bg-white/20 border-white text-white shadow-lg font-bold"
                            : "bg-white/5 border-white/10 text-gray-300 hover:bg-white/10"
                        }`}
                      >
                        <div>
            ...
            <div className="flex justify-between items-center pt-4 border-t border-white/10">
              {step > 1 ? (
                ...
                <button type="button" onClick={onClose} className="secondary-action text-xs py-2.5 px-4">
                  Cancel
                </button>
              )}
              ...
              {step < 5 ? (
                <button
                  ...
                  style={{ backgroundColor: "#FFFFFF", color: "#000000", border: "1px solid #FFFFFF" }}
                  className="primary-action text-xs py-2.5 px-5 flex items-center gap-1.5 bg-white text-black font-bold hover:bg-zinc-200 border border-white cursor-pointer"
                >
