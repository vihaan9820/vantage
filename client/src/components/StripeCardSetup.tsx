    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex flex-col gap-2 pt-2">
        <button
          ...
          className="w-full min-h-[44px] flex items-center justify-center gap-2 font-bold text-xs bg-white hover:bg-zinc-200 text-black rounded-xl transition-all disabled:opacity-50 cursor-pointer shadow-md"
        >
          <Lock size={14} />
  ...
  if (error) {
    return (
      <div className="p-4 rounded-xl bg-slate-900 border border-slate-800 text-center text-xs text-slate-400 space-y-2">
        ...
        {onCancel && (
          <button onClick={onCancel} className="text-white underline text-xs pt-2">
            Go back
          </button>
        )}
      </div>
